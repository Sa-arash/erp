...

{
    ...
    protected static ?string $label = "Building/Warehouse";
    protected static ?string $pluralLabel = "Building/Warehouse";
    ...
    public static function table(Table $table): Table
    {
        ...
        ->query(function (){
            ...
            ->actions([
                Tables\Actions\ReplicateAction::make()->label('Duplicate')->modalHeading('Duplicate Building/Warehouse')
                    ->action(function ($record) {
                        DB::transaction(function () use ($record) {
                            // کپی از رکورد اصلی
                            $new = $record->replicate();
                            $new->title = $record->title;
                            $new->save();

                            // کپی از رابطه structures
                            foreach ($record->structures as $structure) {
                                $newStructure = $structure->replicate();
                                $newStructure->warehouse_id = $new->id; // تنظیم کلید خارجی جدید
                                $newStructure->save();
                            }
                        });
sendSuccessNotification();
                        // پیام موفقیت
//                        filament()->notify('success', 'کپی با موفقیت انجام شد.');
