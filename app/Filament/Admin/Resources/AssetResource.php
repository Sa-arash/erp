    ...
    implements HasShieldPermissions
{
    ...
    {
        return \auth()->user()->can('duplicate_asset'); // TODO: Change the autogenerated stub
    }
        ...
        return $table->query(function () {
            ...
            ->headerActions([
                Tables\Actions\Action::make('auditCheckList')
                    ->label('Print Audit Check List')->form([
                        Select::make('by')->required()->default('warehouse_id')->label('Asset By')->options(['warehouse_id'=>'Location','brand_id'=>'Brand','type'=>'Type','po_number'=>'PO','party_id'=>'Vendor','check_out_to'=>'Employee','check_out_person'=>'Personnel'])->searchable()->preload()
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('Transaction')->iconSize(IconSize::Medium)->form(function ($record) {
                    ...
                    $company=getCompany();
                    $warehouse=Warehouse::query()->firstWhere('id',$data['warehouse_id']);
                    if ($warehouse->employee_id){
                        $record->update(['manager_id'=>$warehouse->employee_id,'warehouse_id'=>$data['warehouse_id'],'structure_id'=>$data['structure_id']]);
                        $assetEmployee=AssetEmployee::query()->firstWhere('employee_id',getEmployee()->id);
                    }
                    if ($assetEmployee){
