...
namespace App\Filament\Admin\Resources\TransationResource\RelationManagers;
...
//                Tables\Columns\TextColumn::make('transactionable.name')->state(fn($record)=>$record->transactionable_type =="App\Models\Expense"  ?   :)->label('Vendor/Customer   ')->alignCenter(),
                Tables\Columns\TextColumn::make('amount_pay')->tooltip(fn($record)=>"Balance Amount : ".$record->balance_amount)->badge()->color(fn($record)=>$record->transactionable_type === "App\Models\Expense" ?  'danger' :"success" )->label('Amount')->alignCenter(),
            ...
            ->filters([
                Tables\Filters\Filter::make('filter')->form([
                  Forms\Components\Section::make([
                      ...
                      Forms\Components\TextInput::make('minBalance')->columnSpan(1)->label('Min balance')->mask(RawJs::make('$money($input)'))->stripCharacters(',')->numeric(),
                      Forms\Components\TextInput::make('maxBalance')->columnSpan(1)->label('Max balance')->mask(RawJs::make('$money($input)'))->stripCharacters(',')->numeric(),
                  ])->columns(4)
