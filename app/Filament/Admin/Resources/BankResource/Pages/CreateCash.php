        ...
        return $form->schema([
            Section::make([
                ...
                TextInput::make('code')->default(function () {
                    if (Bank::query()->where('company_id', getCompany()->id)->where('type',1)->latest()->first()) {return generateNextCode(Bank::query()->where('company_id', getCompany()->id)->latest()->first()->account_code);} else {return "001";}
...
{
    return BankResource::getUrl('editCash',['tenant'=>getCompany()->id,'record'=>$this->record->id]); // TODO: Change the autogenerated stub
}
