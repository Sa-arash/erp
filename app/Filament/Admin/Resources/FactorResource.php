...
class FactorResource extends Resource
{
    ...
    {
        return \auth()->user()->can("create_factor"); // TODO: Change the autogenerated stub
    }
                ...
                Wizard::make([
                    ...
                    Forms\Components\Wizard\Step::make('Invoice')->schema([
                        ...
                        Forms\Components\Repeater::make('items')->required()->relationship('items')->schema([
                            ...
                            Forms\Components\TextInput::make('total')->stripCharacters(',')->mask(RawJs::make('$money($input)'))->live()->readOnly()->default(0)->required()->label('Total'),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                Tables\Actions\ReplicateAction::make()->requiresConfirmation()->action(function ($record){
                    $newInvoice = $record->invoice->replicate();
                    $newInvoice->number=getCompany()->financialPeriods()->where('status', "During")?->first()?->invoices()?->get()->last()?->number+1;
                    $newInvoice->save();

                    $new = $record->replicate();
                    $new->invoice_id=$newInvoice->id;
                    $new->save();
                    foreach ($record->invoice->transactions as $transaction){
                        $transaction = $transaction->replicate();
                        $transaction->invoice_id = $newInvoice->id;
                        $transaction->save();
                    }

                    foreach ($record->items as $item){
                        $item = $item->replicate();
                        $item->factor_id = $new->id;
                        $item->save();
                    }
                    sendSuccessNotification();

                })->tooltip("Duplicate")->label(''),
